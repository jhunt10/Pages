...
func set_mod_data(mod_data):
	...
	if stat_name.begins_with("Bar"):
		...
		var tokens = stat_name.split(":") 
		var display_stat_name =  tokens[0]
		modded_stat_name_label.text = StatHelper.get_stat_abbr(display_stat_name)
		var icon = StatHelper.get_stat_icon(display_stat_name)
		if icon:
	...
	if stat_name.ends_with(":Add"):
		mod_type = BaseStatMod.ModTypes.Add
	if mod_type == BaseStatMod.ModTypes.Add:
		times_label.hide()
		if mod_value < 0:
			mod_value_label.text = str(abs(mod_value))
			plus_label.text = "-"
