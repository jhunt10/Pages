	...
	if event is InputEventMouseButton and (event as InputEventMouseButton).pressed:
		...
		if grid_window_rect.has_point(m_event.global_position):
			...
			if m_event.button_index == 1:
				mouse_pressed = true
				adding = not _editing_areas[_editing_key].has(spot)
				_flip_spot(spot)
...
func _flip_spot(cur_spot):
	if _editing_key == '':
		return
	if adding: 
		_editing_areas[_editing_key].append(cur_spot)
	else:
		_editing_areas[_editing_key].erase(cur_spot)
...
func _sync_tiles():
	...
	if _editing_key != '':
		grid_tile_layer.set_cells_terrain_connect(_editing_areas[_editing_key], 0, 0)
