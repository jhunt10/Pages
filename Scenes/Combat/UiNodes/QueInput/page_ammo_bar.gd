...
var _action:PageItemAction

func _sync():
	if !_action:
		return
	_suppress_sync = true
	ammo_type = _action.get_ammo_type()
	clip_val = _action.get_ammo_max()
	cost_val = _action.get_ammo_cost_per_use()
	current_val = _action.get_ammo_current()
