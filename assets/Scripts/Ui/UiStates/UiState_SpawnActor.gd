...
func _init(controler:UiStateController, args:Dictionary) -> void:
	...
	placing_actor_id = args.get("SpawningActorId")
...
func _mouse_moved_into_spot(spot):
	if state == States.Placing:
		...
		_put_actor_in_spot(placing_actor_id, spot, is_spot_valid)
		...
		set_actor_rotation(placing_actor_id, dir)
...
func _mouse_button_down_in_spot(spot):
	if state == States.Placing:
		if _is_spot_valid(spot):
			...
			_put_actor_in_spot(placing_actor_id, mouse_down_spot)
...
func _put_actor_in_spot(actor_id, spot, is_valid:bool = true):
	var actor = ActorLibrary.get_actor(actor_id)
	...
	if not actor_node:
		return
	if actor_node:
		actor_node.position = CombatRootControl.Instance.MapController.actor_tile_map.map_to_local(spot)
		actor_node.show()
	if is_valid:
...
func set_actor_rotation(placing_actor_id, direction):
	actor_positions[placing_actor_id].dir = direction
