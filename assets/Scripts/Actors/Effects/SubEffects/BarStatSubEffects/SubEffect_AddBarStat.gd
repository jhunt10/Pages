...
func get_active_stat_mods(effect:BaseEffect, subeffect_data:Dictionary)->Array:
	...
	var max_bar_mod = BaseStatMod.new(effect.Id, "BarMax:"+bar_stat_name, effect.get_display_name(), BaseStatMod.ModTypes.Add, max_value)
	var regen_bar_mod = BaseStatMod.new(effect.Id, "BarRegen:"+bar_stat_name+":Round", effect.get_display_name(), BaseStatMod.ModTypes.Add, regen_value)
