...
func _init(moving_actor:BaseActor, blocking_actor:BaseActor, winning_actor:BaseActor, damage_event_val:DamageEvent, atk_mods:Dictionary) -> void:
	moving_actor_id = moving_actor.Id
	blocking_actor_id = blocking_actor.Id
	self.damage_event = damage_event_val
