class_name PageSpriteEditControl
extends Control
...
func load_page_data(data:Dictionary):
	if not connected:
		connected = true
		PageEditControl.Instance.file_edit_control.file_changed.connect(_on_file_path_change)
...
func _sync_sprites():
	var load_path:String = PageEditControl.Instance.file_edit_control.get_load_path()
...
func get_sprite_options()->Array:
	if PageEditControl.Instance and PageEditControl.Instance.file_edit_control:
		return _search_for_sprites(PageEditControl.Instance.file_edit_control.get_load_path())
	return []
