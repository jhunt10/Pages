...
func do_thing(parent_action:PageItemAction, subaction_data:Dictionary, que_exe_data:QueExecutionData,
	...
	if damage_data_key:
		zone_data['DamageDatas'] =  parent_action.get_damage_datas(actor, damage_data_key)
	
	zone_data['AttackDetails'] = parent_action.get_attack_details()
	zone_data['EffectDatas'] = parent_action.action_data.get("EffectDatas", {}).duplicate()
